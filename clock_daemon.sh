...
#brightness=100
#brightness=50
brightness=40  # Seems the most stable and not too bright
...
sudo ${basedir}/clock --led-gpio-mapping=adafruit-hat --led-chain=2 --led-slowdown-gpio=2 -b ${brightness} -f ${basedir}/fonts/7x14B.bdf -x 5 -y 0.5 -C ${color} --led-brightness=${brightness} --led-pwm-bits=8 -d '%I:%M %p' -t ${bottomtext} -L ${bottomcolor} & 
